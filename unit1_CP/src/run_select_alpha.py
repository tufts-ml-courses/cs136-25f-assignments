...
def calc_per_word_log_evidence(estimator, word_list):
    ...
    # Hint: use gammaln (imported above) instead of separate log function calls
    log_evidence = 0.0
...
if __name__ == '__main__':
    ...
    for nn, N in enumerate(n_train_list):
        ...
        for aa, alpha in enumerate(alpha_list):
            estimator = PosteriorPredictiveEstimator(vocab, alpha=alpha)

            train_log_ev_list[aa] = calc_per_word_log_evidence(
                estimator, train_word_list[:N])

            estimator.fit(train_word_list[:N])
            test_log_lik_list[aa] = estimator.score(test_word_list)
