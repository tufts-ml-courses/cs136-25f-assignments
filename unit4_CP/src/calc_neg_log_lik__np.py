...
def calc_neg_log_lik(x_ND, log_pi_K, mu_KD, stddev_KD):
    ...
    ## TODO compute joint probability of x and z given parameters
    logp_z_and_x_NK = np.zeros((N, K))
    for k in range(K):
        # TODO fill in column k of array logp_z_and_x_NK
        # so that entry [n,k] = log p(x_n, z_n = k | parameters)

        # Hint 1: log p( z_n = k ) is just log_pi_K[k]
        # Hint 2: call scipy.stats.norm.logpdf to get log p(x_n| z_n=k, params)
        pass

    # TODO call logsumexp, with appropriate axis kwarg
    # want an array where n-th entry = log p(x_n | parameters)
    logp_x_N = np.sum(mu_KD, axis=0) # TODO FIXME
