...
Examples
========
>>> np.set_printoptions(formatter={'float':lambda x: '%.3f' % x}, linewidth=80)
>>> D = 2

## Verify that variance penalty works as expected
# Empty components (with no assigned data) 
# should have variance equal to the intended "mode" of the penalty
# We'll use a mode of 2.0 (so stddev = sqrt(2.0) = 1.414...)
>>> gmm_em = GMM_PenalizedMLEstimator_EM(K=3, D=2, seed=42,
...     variance_penalty_mode=2.0)
>>> empty_ND = np.zeros((0,D))
>>> log_pi_K, mu_KD, stddev_KD = gmm_em.generate_initial_parameters(empty_ND)
>>> float(calc_neg_log_lik(empty_ND, log_pi_K, mu_KD, stddev_KD))
-0.0
...
# Set pi,mean,stddev to 'true' params that generated the data
...
# Test the E-step
# Verify first 3 examples belong to cluster 1
>>> gmm_em.estep__calc_r_NK(x_ND[:3])
array([[1.000, 0.000, 0.000],
       [1.000, 0.000, 0.000],
       [1.000, 0.000, 0.000]])

# Verify last 3 examples belong to cluster 3
>>> gmm_em.estep__calc_r_NK(x_ND[-3:])
...
# Test the M-step
>>> r_NK = gmm_em.estep__calc_r_NK(x_ND)
>>> log_pi_K = gmm_em.mstep__update_log_pi_K(r_NK)
>>> np.exp(log_pi_K)
array([0.333, 0.333, 0.333])

>>> mu_KD = gmm_em.mstep__update_mu_KD(r_NK, x_ND)
>>> mu_KD
array([[-0.007, 0.010],
       [-1.008, 0.009],
       [-0.005, 1.005]])
...
>>> gmm_em.mu_KD = mu_KD
>>> stddev_KD = gmm_em.mstep__update_stddev_KD(r_NK, x_ND)
>>> stddev_KD
array([[0.076, 0.091],
       [0.098, 0.103],
       [0.240, 0.042]])

## Test entire learning process
>>> gmm_em = GMM_PenalizedMLEstimator_EM(
...
class GMM_PenalizedMLEstimator_EM(GMM_PenalizedMLEstimator):
    ...
    * Initialization method generate_initial_parameters() from parent as well
    ...
    def estep__calc_r_NK(self, x_ND):
        ...
        r_NK[:,0] = 1.0 # TODO FIXME
        for k in range(self.K):
            # TODO update k-th col. of r_NK to optimal value given GMM params
            pass
    ...
    def mstep__update_log_pi_K(self, r_NK):
        ...
        log_pi_K : 1D array, shape (K,)
            ...
            Must satisfy logsumexp(log_pi_K) == 0.0 (which means sum(exp(log_pi_K)) == 1.0)
        '''
        pi_K = np.sum(r_NK, axis=0) / r_NK.shape[0]
        assert np.allclose(np.sum(pi_K), 1.0)
        log_pi_K = np.log(1e-13 + pi_K) # small positive constant avoids log(0)
        return log_pi_K
    ...
    def mstep__update_mu_KD(self, r_NK, x_ND):
        ...
        mu_KD = np.zeros((self.K, self.D)) # TODO FIXME
        return mu_KD
    ...
    def mstep__update_stddev_KD(self, r_NK, x_ND):
        ...
        # TODO Calc weighted sum of squared differences for each k,d pair
        ssd_KD = np.zeros((self.K, self.D)) 
        for k in range(self.K):
            ssd_KD[k,:] = 0.0 # TODO FIXME
        # TODO compute sum of r_NK for each k
        N_K1 = np.ones((self.K, 1)) # TODO FIXME
    ...
    def fit(self, x_ND, x_valid_ND=None, verbose=True):
        ...
        Ntrain, D = x_ND.shape
        n_pixel_train = np.maximum(Ntrain, 1) * D
        if x_valid_ND is not None:
            Nvalid, _ = x_valid_ND.shape
            n_pixel_valid = np.maximum(Nvalid, 1) * D
        ...
        for iter_id in range(self.max_iter + 1):
            ...
            ## Loss step : compute log likelihood on train set
            tr_score = self.score(x_ND)
            tr_score_per_pixel = tr_score / n_pixel_train
            self.history['iter'].append(iter_id)
            self.history['train_score_per_pixel'].append(tr_score_per_pixel)

            loss_with_penalty = -1.0 * tr_score + self.calc_penalty_stddev()
            self.history['train_loss'].append(loss_with_penalty)
            self.history['train_loss_per_pixel'].append(loss_with_penalty / n_pixel_train)

            if x_valid_ND is None:
                va_score_message = ""
            else:
                # TODO compute the per-pixel log likelihood on validation set
                # You can use built-in `score` method, like for tr_score above
                # Divide by `n_pixel_valid`, the number of pixels in val set
                va_score_per_pixel = 0.0123 + self.seed / 10000.0 # TODO FIXME
                self.history['valid_score_per_pixel'].append(va_score_per_pixel)
                va_score_message = "| valid score %9.6f" % (
                    self.history['valid_score_per_pixel'][-1])
            ...
            if self.do_double_check_correctness:
                ...
                # TODO this assertion should pass if code is correct
            ...
            if self.do_double_check_correctness:
                ...
                # TODO this assertion should pass if code is correct
                # assert loss_m <= loss_e + 1e-9 # verify loss never goes up
